...
def index(request):
    shades= Products.objects.filter(
        category="Shades").filter(pro_rating__gte=4.3)[:8]
    watches = Products.objects.filter(
        category="Watches").filter(pro_rating__gt=4)[:8]
    ...
    return render(request, 'index.html', {"shades": shades, "watches": watches, "blog": blog, "review": review, "banner": banner})
...
def billing(request, id):
    ...
    if request.method == "POST":
        ...
        query2 = DeliverProducts.objects.create(username=request.user, product=query, quantity=int(
            quantity), paymentmod="COD", total_amount=totalamount, Status="pending")
